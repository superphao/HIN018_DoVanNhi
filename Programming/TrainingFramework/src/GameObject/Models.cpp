...
int Models::Init(const char *szFileName, GLuint type)
{
	...
	glBufferData(GL_ARRAY_BUFFER, iNumVertex* sizeof(Vertex), verticesData, GL_DYNAMIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, m_iVboID);
	glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(Vertex), verticesData);
	...
	//if(m_iIboID == 0)
	{
		...
		glBufferData(GL_ELEMENT_ARRAY_BUFFER,m_iNumIndices *sizeof(GLuint), Indices, GL_DYNAMIC_DRAW);
